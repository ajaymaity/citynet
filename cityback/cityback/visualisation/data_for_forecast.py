...
def getDataForTheLastWeek():
    ...
    DublinBikesStationRealTimeUpdate.objects.filter(
        station_last_update__gte=monday_of_last_week,
        station_last_update__lt=monday_of_this_week)
...
def getDataforTheLastNdays(ndays=30):
    ...
    items = DublinBikesStationRealTimeUpdate.objects.filter(
        station_last_update__gte=last_month)
    return items
...
def writeToCsv(data, filename="output.csv"):
    ...
    with open(filename, 'w') as fcsv:
        ...
        for d in data:
            csvf.writerow([d.station_last_update.strftime(FORMAT),
                           d.available_bike_stands * 100 / d.bike_stands])
