...
def averageModel():
    ...
    distinct_stations = all_stations.order_by("parent_station__station_number").\
        distinct("parent_station__station_number")
    start = time.time()
    for station in distinct_stations:
        print("***** ", station.parent_station_id, " *****")
        stations = all_stations.filter(parent_station__station_number__exact=station.parent_station_id)

        objs = stations.order_by("station_last_update")
        groups = itertools.groupby(objs, lambda x: date_hour_min(x.station_last_update))
